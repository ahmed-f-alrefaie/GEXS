...
void VoigtProfile::ExecuteCrossSection(){
	...
	double gammaHW = LORENTZ_CUTOFF;//std::min(25.0*input->GetPressure(),100.0);
	int temp_ib;
	int temp_ie;
	int temp_points;
	int max_points = input->GetMaxPoints();
	max_points = max((int)((gammaHW)*3.0/dfreq),max_points);
	...
	for(int  i = 0; i < num_trans_files; i++){
		...
		while(state_reader->ReadNextState(t_nu,t_gns,t_ei,t_aif,t_gammaL,t_n)){
			...
			//if(
			Ntrans++;
			//If we're not ready for work, continue reading one at a time since IO is a cruel cruel beast compared to computation
