...
void Timer::EndTimer(std::string name){
	...
	if(!temp.started){
		...
		temp.last_time = time_taken_seconds;
	//	temp.run_idx++;
		temp.running_avg = (time_taken_seconds + (float)(time_data[name].calls-1)*temp.running_avg)/time_data[name].calls;
		//if(temp.run_idx==RUNNING_AVG_TOT) temp.run_idx = 0;
		time_data[name] = temp;
		
		//time_data[name].running_avg[temp.run_idx] = time_taken_seconds;
...
float Timer::GetRunningAvgTimeInSeconds(std::string name){
	if(!timer_exists(name)){
		fprintf(stderr,"[Timer::GetAvgTimeInSeconds]: Timer of name %s does not exist!!\n",name.c_str());
		//exit(0);	
		return -1.0;
	...
	//float avg = 0.0f;
	//for(int i = 0; i < RUNNING_AVG_TOT; i++)
	//	avg+=time_data[name].running_avg[i];
	//avg /= (float)RUNNING_AVG_TOT;
	return time_data[name].running_avg;
};

float Timer::GetLastTimeinSeconds(std::string name){
	if(!timer_exists(name)){
		//fprintf(stderr,"[Timer::GetAvgTimeInSeconds]: Timer of name %s does not exist!!\n",name.c_str());
		//exit(0);	
		return -1.0f;
	}
	return time_data[name].last_time;
};
...
void Timer::initialize_timer(const std::string name){
	...
	if( !timer_exists(name) ){
		...
		time_data[name].last_time = 0.0f;
		time_data[name].run_idx = 0;
		time_data[name].running_avg = 0.0f;
